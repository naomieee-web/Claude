...
export default function Team() {
  return (
    <section className="bg-white border-t border-gray-100 px-4 md:px-12 lg:px-16 py-12 lg:py-16 pb-20 lg:pb-32">
      ...
      <div className="hidden md:flex flex-row items-end justify-center gap-3 mt-8">
      ...
      <div className="flex md:hidden overflow-x-auto snap-x snap-mandatory gap-4 pb-4 scrollbar-hide mt-8 w-full">
        {members.map((member) => (
          <div
            key={member.name}
            className="snap-center flex-shrink-0 w-64 h-80 rounded-2xl overflow-hidden relative"
          >
            <img
              src={member.img}
              alt={member.name}
      ...
      <div className="flex justify-center gap-2 mt-4 md:hidden">
        {members.map((member) => (
          <span key={member.name} className="w-2 h-2 rounded-full bg-gray-300" />
        ))}
      </div>
    </section>
  )
